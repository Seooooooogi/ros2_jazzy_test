...
## L-009: DDS 수신버퍼 튜닝 — `rmem_max`(천장) ≠ 실제 수신버퍼, 하한은 "최대 토픽 1프레임" (2026-06-04)

- **Symptom**: L-008 의 CycloneDDS 0Hz 를 풀려고 `net.core.rmem_max` 만 키우고 끝내 효과 0, 또는 버퍼를 토픽 실제 크기와 무관하게 잡아 color/depth 는 받아지는데 `depth/color/points`(pointcloud)만 계속 0Hz. "버퍼 올렸는데 왜 안 되지" 로 헤맴.
- **Why it happened**: 두 값의 역할 혼동. (1) `net.core.rmem_max` 는 **커널 상한(천장)** — 소켓이 `SO_RCVBUF` 로 요청 가능한 최대치일 뿐 실제 할당이 아님. (2) 실제 수신버퍼 = DDS 가 거는 `SO_RCVBUF` 요청값 = CycloneDDS `SocketReceiveBufferSize`(예: rcvbuf 10MB). 요청값 > 천장이면 **천장으로 깎임(clamp)** → `rmem_max` 만 올리고 rcvbuf 안 키우면 DDS 가 여전히 작게 요청해 무효, rcvbuf 만 키우고 천장이 208KB 면 깎여서 무효. **둘 다** 충족돼야 함. (3) UDP 수신버퍼는 단일 프레임의 fragment 를 다 담아야 재조립 성공 → **버퍼 하한 = 수신 토픽 중 가장 큰 1프레임 크기**. 1280×720 기준: RGB8 ≈ 2.76MB, depth Z16 ≈ 1.84MB, **pointcloud(XYZ+RGB, ~16B/point) ≈ 14.7MB**. rcvbuf 10MB 는 color/depth 1프레임 + 지터 여유(≈3~4프레임)는 되지만 **pointcloud 1프레임(14.7MB) > 10MB** 라 pointcloud 만 재조립 실패.
- **Correction**: ① 30fps 를 직접 만든 건 **rcvbuf(`SocketReceiveBufferSize`)** — 1프레임 재조립 + 슬로우 컨슈머(`ros2 topic hz` 등) 지터 흡수. `rmem_max` 는 그게 안 깎이게 하는 **전제(enabler)** 일 뿐(2GB 는 토픽 늘려도 신경 끄려는 넉넉한 천장이지 그 자체가 버퍼 아님). ② rcvbuf 는 **수신할 가장 큰 토픽 1프레임 + 지터 여유**로 잡는다 — pointcloud 받으려면 수십 MB(예: 32~64MB), `rmem_max ≥ rcvbuf` 유지. ③ **Fast-DDS 기본(같은 호스트 판정=SHM)엔 `rmem_max`/socket 버퍼가 무효** — SHM 은 UDP 를 안 타므로 SHM segment/pool·QoS history depth 가 노브. socket 버퍼 튜닝은 Fast-DDS 가 **UDP 로 도는 경우**에만 CycloneDDS 와 동일하게 먹힘 — 즉 ⓐ 서로 다른 머신(호스트 경계), ⓑ 컨테이너 **bridge 네트워크**(다른 IP→다른 호스트 판정→UDP), ⓒ SHM 명시적 off(`useBuiltinTransports=false`+UDPv4 transport). **[정정 2026-07-16 실측]** 기존에 "컨테이너 IPC 격리"를 UDP 조건으로 적었으나 오류: `network_mode: host` 컨테이너는 `/dev/shm` 이 격리돼도 Fast-DDS 가 여전히 **같은 호스트로 판정→SHM 선택→조용히 데이터 유실**(discovery 는 UDP 라 토픽 목록엔 보이는데 내용 0Hz, 소형·14MB 무관). **IPC 격리 자체는 UDP fallback 을 유발하지 않는다** — UDP 는 peer 가 **다른 IP** 일 때만(bridge / 별도 머신). host 모드에서 FastDDS 를 살리려면 ⓒ(SHM off)가 필요. ④ `depth/color/points` 는 카메라가 USB 로 보내는 게 아니라 ros wrapper(host)가 depth+color 로 **합성**하는 토픽 → **USB 대역폭 무관, DDS socket 부담은 최대**. 버퍼 산정 기준은 항상 pointcloud.
- **Trigger**: `net.core.rmem_max`/`SocketReceiveBufferSize` 등 DDS 수신버퍼 튜닝, 대용량 토픽(특히 pointcloud)만 0Hz, "버퍼 올렸는데 효과 없음", RMW 별 버퍼 노브 차이(Fast-DDS SHM vs CycloneDDS UDP), 토픽 추가 후 일부만 수신 실패.

---

## L-010: 컨테이너 import smoke 통과 ≠ 노드 런타임 동작 — e2e service 왕복이라야 잡힌다 (2026-06-08)

- **Symptom**: `containers/build-all.sh` 의 yolo import smoke(`python3 -c "import torch, ultralytics, object_detection.yolo ..."`)는 통과했는데, 실제 `ros2 run object_detection object_detection` 으로 노드를 띄우니 ① `ModuleNotFoundError: ultralytics`(기동 즉시), ② service 호출 시 `RuntimeError: Executor is already spinning`(크래시) 가 연달아 터짐.
- **Why it happened**: import smoke 가 실제 실행 경로와 다른 환경/코드 경로를 탐. ① smoke 는 `python3`(PATH 상 venv python)로 import 하지만 `ros2 run` 은 ament 콘솔스크립트 shebang(`/usr/bin/python3` 시스템)으로 실행 — colcon build 가 venv 생성 전이라 shebang 이 시스템 python 으로 굳어 venv 의 ultralytics 미발견. ② import 만으론 노드 init/콜백을 안 타므로 서비스 콜백 안의 `rclpy.spin_once(img_node)`(메인 `rclpy.spin` 과 재진입 충돌)가 안 드러남. **둘 다 service 를 실제 호출하는 e2e 라야 노출.**
- **Correction**: 컨테이너 검증을 import smoke 로 끝내지 말고 **실제 노드 기동 + service 왕복(e2e)** 까지 본다. import 되는 python 과 `ros2 run` 이 쓰는 python(shebang)이 같은지 확인. 같은 패턴 결함(spin_once 재진입 등)이 한 파일에서 보이면 **패키지 전체를 grep 해 전수 수정 후** 재빌드 — 한 파일만 고치고 (비싼) 재빌드하면 나머지에서 또 터져 torch 재다운로드를 반복함.
- **Trigger**: 컨테이너/ROS2 노드 검증, import 는 되는데 `ros2 run` 실패, "smoke 통과인데 런타임 크래시", venv+colcon shebang 불일치, 서비스 콜백 내 spin, 같은 버그가 여러 파일에 퍼진 정황.
