...
robot_control(실제 pick 모션 + 무한 루프)은 안전상 기본 제외한다. 별도 터미널에서
수동 실행하거나(권장), start_robot_control:=true 로 명시 옵트인해야 기동된다.
...
안전 경고:
  ...
  - start_robot_control:=true + mode:=real 이면 ~8초 뒤 실기가 movej 로 물리 이동한다. 비상정지 대기.

사용 예:
  # 에뮬레이터 + 카메라/컨테이너 없이 드라이버만(이미지·카메라 불필요한 점검)
  ros2 launch <repo>/cobot2_ws/launch/bringup_all.launch.py mode:=virtual camera:=false containers:=false
  # 실기 + 카메라 + 컨테이너(이미지 빌드/.env/cyclonedds.xml 준비 후)
  ros2 launch <repo>/cobot2_ws/launch/bringup_all.launch.py mode:=real host:=192.168.1.100
"""
import os
...
# 설치 레포 루트 — 이 파일은 <repo>/cobot2_ws/launch/ 에 있다(부모의 부모).
# 후속 패키지화로 robot_control/share/ 에 설치하면 __file__ 위치가 바뀌므로 그때는
# REPO 계산을 ament_index(get_package_share_directory) 기반으로 교체해야 한다 — 현 standalone 실행은 정상.
...
def generate_launch_description():
    args = [
        ...
        DeclareLaunchArgument(
            "start_robot_control", default_value="false",
            description="DANGER: true + mode:=real 이면 ~8초 뒤 실기가 물리 이동(movej)",
        ),
    ]
    ...
    # robot_control = 실제 pick 모션 + 무한 루프. 기본 제외(안전). 옵트인 시 드라이버/컨테이너
    # service server 가 뜰 시간을 준 뒤 기동(8초는 휴리스틱 — robot_control 자체의 wait_for_service 가 실 안전망).
    robot_control = TimerAction(
        period=8.0,
        actions=[Node(
    ...
    return LaunchDescription(
        args + [robot_bringup, realsense, compose_up, compose_down, robot_control]
    )
