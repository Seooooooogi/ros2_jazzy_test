...
- **`setup-app.sh`(신규, 통합)**: 워크스페이스(`obtain_cobot2` → `dsr-project-install.sh` → `realsense-install.sh` → `colcon-build.sh`) + 컨테이너(`nvidia-container-toolkit-install.sh` → `containers/fetch-images.sh` → `openai-key-setup.sh`). 플래그 `--workspace-only`/`--containers-only`/`--reset`/`--build`. `reinstall-workspace.sh` 흡수·폐기. ("처음부터 다시" 플래그는 install.sh 와 이름 통일 = `--reset`: install.sh 는 state 초기화, setup-app 은 doosan 클론+빌드 산출물 wipe 후 재빌드.)
