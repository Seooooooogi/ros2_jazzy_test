...
| Voice (langchain/openai/sounddevice/numpy) | **host 직접 설치**(system pip `--break-system-packages`, ADR-027) — 컨테이너 아님 | `resources/app-install.sh voice` | 마이크 하드웨어 종속이라 host 실행. OPENAI 키 = **인스톨러가 다루지 않는다**(ADR-028 의 `openai-key-setup.sh` 제거·인스톨러 자동생성 없음은 유효하나, 같은 ADR 의 `~/.config/cobot2/.env` 이관은 2026-07-10 폐기) — 사용자가 `voice_processing` 패키지의 `resource/.env` 에 직접 배치하고 노드가 `load_dotenv` 로 읽는다(그 파일은 colcon 빌드에 포함) |
...
> feature 모델(melspectrogram/embedding/VAD)은 wheel 미동봉. 초기(ADR-014)엔 `download_models()` 로 받았으나, transient 504 시 에러 HTML 을 `.tflite` 로 저장해 런타임에 크래시했다 → 지금은 3단계다: (1) `resources/oww_models/` 동봉본(melspectrogram/embedding/silero_vad — 동봉본이 authoritative)을 openwakeword 설치 경로에 **먼저** 복사, (2) 그 다음 `download_models()` 를 호출해 나머지 stock 모델만 받는다(이미 있는 feature 모델은 존재-가드로 skip — **다운로드 경로는 폐기된 게 아니라 여전히 쓰인다**), (3) 설치 경로의 모든 `.tflite` 에 `TFL3` 식별자(offset 4)를 검증해 손상본(504 HTML 등)은 삭제하고 fail-loud — 재실행 시 다시 받는다.
