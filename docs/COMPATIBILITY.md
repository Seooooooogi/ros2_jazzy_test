...
| Voice (langchain/openai/sounddevice/numpy) | **host 직접 설치**(system pip `--break-system-packages`, ADR-027) — 컨테이너 아님 | `resources/voice-host-install.sh` | 마이크 하드웨어 종속이라 host 실행. OPENAI 키 = 사용자가 `~/.config/cobot2/.env` 직접 생성(ADR-028 — `openai-key-setup.sh` 제거·인스톨러 자동생성 없음), `bringup.sh` 가 로드 |
...
> **토픽 이름 변경 + remap 배선 (2026-07-21)**: host publish 경로가 `/camera/camera/*` → **`/camera/*`** 로 바뀌었다(중복 namespace 제거 — 위 프로파일 행). 종전의 "토픽 이름 동일 → 구독 경로 무변경" 서술은 이 변경으로 폐기. 컨테이너 `object_detection` 의 `ImgNode` 는 절대 경로 대신 **상대 이름**(`color/image_raw`, `aligned_depth_to_color/image_raw`, `color/camera_info`)을 구독하고, `containers/bringup.sh:140` 이 기동 시 `--ros-args -r img_node:__ns:=/camera` 를 붙여 `/camera/*` 로 해석시킨다.
...
> - 위험: remap 을 빠뜨려도 노드는 정상 기동하고 **에러 없이 토픽만 조용히 빈다**. 또한 소비자 소스(`~/cobot_ws/src/cobot2`)는 이 레포도 다른 어떤 git 레포도 추적하지 않으므로, 프로듀서만 갱신되고 소비자 패치가 전달되지 않으면 파이프라인이 통째로 죽는다.
> - 구 `cobot2_bringup/bringup_all.launch.py` 는 여전히 upstream `examples/align_depth/rs_align_depth_launch.py` 를 include 해 `/camera/camera/*` 를 낸다 — 그걸 쓸 때는 remap 을 `-r img_node:__ns:=/camera/camera` 로 맞춰야 한다.
