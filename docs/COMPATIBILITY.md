...
| RMW (DDS) | **`rmw_cyclonedds_cpp` 표준** — `ros-jazzy-cyclonedds 0.10.5`, `ros-jazzy-rmw-cyclonedds-cpp 2.2.3` (실측 2026-06-05) | `resources/config.sh` (`RMW_IMPLEMENTATION`); `resources/dds-tuning.sh`; ADR-016 | fastrtps→cyclonedds 전환(ADR-016). 대용량 토픽 수신엔 **커널 sysctl 버퍼 + XML(`CYCLONEDDS_URI`) 버퍼가 세트** 필요(`/etc/sysctl.d/60-cyclonedds.conf` rmem/wmem 2GB + `SocketReceive/SendBufferSize 64MB`). XML 은 설치 머신 유선 NIC 를 자동 whitelist(wifi 제외). host↔컨테이너는 동일 RMW + `ROS_DOMAIN_ID`(기본 42) + `network_mode: host`(커널버퍼·NIC 상속) |
