...
| Voice (langchain/openai/sounddevice/numpy) | `application-containers`: host 미설치(컨테이너 전용) · `application-shell`: **host venv** (아래 섹션, ADR-014) | `resources/openai-key-setup.sh` / `host-python-deps.sh` | 컨테이너 변종은 host pip 없음, install.sh 마지막 step 은 `.env` 의 `OPENAI_API_KEY` 점검/입력만(host 설치 없음). shell 변종은 host venv 에 설치 후 `ros2 run` |
