...
#
# a01~a04 오케스트레이터의 모든 step 을 하나의 연속 시퀀스([n/11])로 실행한다.
...
#
# reboot(step 5) 후에는 다시 'bash install.sh' 를 실행하면 step 6 부터 이어진다.
set -euo pipefail
...
usage() {
    cat <<'EOF'
install.sh — host 셋업 단일 진입점 (a01~a04 통합, 전체 11 step)
...
reboot(step 5) 후에는 다시 'bash install.sh' 를 실행하면 step 6 부터 이어집니다.
...
# --- step 1~4: 사전준비 (a01: NVIDIA / Docker / ROS2 jazzy / extras) ---
...
# --- step 5: reboot 경계 (a01) ---
# run_step 으로 감싸지 못한다: reboot 은 프로세스를 종료하고, 이후 step 6~11 은 재부팅 후
...
if ! step_should_skip a01_reboot; then
    ...
    confirm_or_abort "사전준비(드라이버/Docker/ROS2) 완료. 드라이버·docker 그룹 적용을 위해 지금 재부팅할까요?"
    step_end_ok
    echo
    echo ">>> 재부팅합니다. 복귀 후 'bash install.sh' 를 다시 실행하면 step 6 부터 이어집니다."
    sudo reboot
fi

# --- step 6~9: 로봇/카메라 (a02: DSR + RealSense + colcon 빌드) ---
run_step 6 a02_dsr_project   bash "${RESOURCE_DIR}/dsr-project-install.sh"
run_step 7 a02_realsense_sdk bash "${RESOURCE_DIR}/realsense-sdk-install.sh"
run_step 8 a02_realsense_ros bash "${RESOURCE_DIR}/realsense-ros-install.sh"
run_step 9 a02_colcon_build  bash "${RESOURCE_DIR}/colcon-build.sh"

# --- step 10: 개발 도구 (a03: VS Code) ---
run_step 10 a03_vscode bash "${RESOURCE_DIR}/vscode-install.sh"

# --- step 11: 음성 사전 점검 (a04: .env 자격증명 + Docker Hub 로그인 안내) ---
run_step 11 a04_voice_env bash "${RESOURCE_DIR}/voice-env-check.sh"

state_dump
echo "install: 전체 11 step 완료 — host 셋업 종료."
