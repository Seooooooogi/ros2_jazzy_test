...
**(2026-06-29 [실측] 세션 #2 반영) — corecode 전 트랙 스모크 PASS(robot/camera/voice/yolo) + verify.py 인터랙티브 hand-off.**
...
- **⚠ verify.py = 인터랙티브 실모션 — 자율 실행 불가(다음 [실측] OPEN)**: 자동 모션 없음. cv2 `"Webcam"` 창에서 **운영자가 픽셀 좌클릭 → 그 점으로 movel→하강(-20)→그리퍼 close→상승(+20)→movej(JReady)→open**(사람이 매 동작 트리거). `T_gripper2camera.npy` 존재 → 즉시 가능하나 **실로봇 motion** 이라 작업공간 클리어 + e-stop + 저속(`VELOCITY,ACC=60` → 하향 권장) 확인 후 사용자가 클릭으로 구동. 기동: 위 bringup + `python3 corecode/Calibration_Tutorial/verify.py`(GUI 세션·host 실행). 스모크 후 bringup teardown 완료.
- **⚠ working tree 미커밋(사용자 측 — 내 작업 아님, 미확인)**: `install.sh`·`resources/interaction.sh`·`setup-app.sh` 에 sudo 키프라이브를 `interaction.sh::sudo_prime` 로 추출·공유하는 리팩토링이 uncommitted. 정체/완성도 사용자 확인 대기 — 핸드오프 커밋엔 미포함(handoff 파일만 stage).
