...
## Last updated
2026-06-22 ([실측]) — **[실측]** origin 동기화 + voice 컨테이너 dev 빌드 실패 수정 + 컨테이너 이름 고정. 전부 `feat/application-containers` push(HEAD `b1887d0`, origin 동기 — 원격 [문서] README 2커밋 `542a4aa`/`78bf46a` 위로 rebase, README 충돌은 원격 새 구조 채택 + 테스트 줄 재배치로 해소):
