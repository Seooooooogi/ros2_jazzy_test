...
- **⚠ [문서] Notion ↔ 레포 워크스페이스 경로 diverge(승인 대기)**: Notion 3페이지를 `~/cobot_ws` → **`~/cobot2_ws`**, venv 페이지를 `cobot_demo_ws` → **`cobot_venv_ws`** 로 전수 변경했으나 **레포는 `~/cobot_ws` 그대로**(`resources/config.sh:49` `DSR_WORKSPACE` 기본값, 참조 파일 20+). 수동 설치 경로만 쓰면 즉시 깨지진 않지만(스크립트 미경유) 두 문서가 다른 경로를 지시하는 상태. 레포도 맞출지 / Notion 을 되돌릴지 / `DSR_WORKSPACE` override 안내로 덮을지 사용자 결정 필요.
