...
**(2026-06-30 [문서] 세션 반영) — ROS_DOMAIN_ID 입력 옵션화 + prompt 를 setup-app 끝으로 이동·영어화.**
- **구현**: 디폴트 42 하드코딩 → 사용자 선택값을 `~/.config/ros2_jazzy_test/ros_domain_id`(XDG, `--reset` 보존) 에 영속, `config.sh` 가 그 파일을 디폴트로 읽음(우선순위 **env > 파일 > 42**). prompt 는 **setup-app.sh 컨테이너 단계 끝(OPENAI 키 바로 앞), 영어 안내** — OPENAI 와 동일 패턴. install.sh(base) 는 도메인 안 물음(완전 자동, confirm 만). 도메인은 host↔컨테이너 통신용이라 컨테이너 등장 시점(setup-app)이 기능적으로도 맞음.
- **bashrc 동적 읽기 (핵심 트릭)**: `dds-tuning.sh`(install.sh step 8) 가 `~/.bashrc` managed block 에 도메인을 **파일 동적 읽기 줄**로 심음(`export ROS_DOMAIN_ID="$(cat .../ros_domain_id 2>/dev/null)"; export ROS_DOMAIN_ID="${ROS_DOMAIN_ID:-42}"`) → setup-app 에서 나중에 값을 정해도 **새 터미널이 자동 반영, bashrc 재작성 불필요**. 빈/없는 파일 모두 42(config.sh 와 parity). compose 무변경(config.sh export env 수신).
- **변경 파일**: `interaction.sh`(prompt 영어+주석) / `install.sh`(시작 prompt 호출 제거) / `dds-tuning.sh`(동적 줄, SC2016 의도적 disable) / `setup-app.sh`(컨테이너 단계 도메인 step + 진행률 +3→+4) / `config.sh`(주석) / 문서(.env.example·README·COMPATIBILITY). 이전 fix 2(octal 선행0, docker run continuation) 유지.
- **[실측] 검증 대기**(reboot/컨테이너 동반이라 이 세션 미실행): `bash install.sh`(도메인 안 물음, 자동) → reboot → `bash setup-app.sh` → 컨테이너 단계 끝에서 도메인 입력(예 30) → 새 터미널 `echo $ROS_DOMAIN_ID`=30 + `grep ROS_DOMAIN_ID ~/.bashrc`(동적 줄) → `set -a; source resources/config.sh; set +a; docker compose -f containers/docker-compose.yml config | grep ROS_DOMAIN_ID` 세 서비스 동일 → `--reset` 후 `~/.config/ros2_jazzy_test/ros_domain_id` 보존.
- **설계 결정**: read-time 범위 재검증 = defer(env/파일 신뢰=CI override, hand-edit=GIGO). opus 리뷰 2회(옵션화 whole-branch + 재배치) = Ready to merge/commit(Critical·Important 0). ⚠ `docs/specs|plans/2026-06-30-ros-domain-id-option*` 의 "install.sh 시작 prompt" 기술은 이 재배치로 superseded — dated artifact 라 미갱신(설계 당시 기록).
- **세션 중 git 주의**: 옵션화 1차분(T1~T3+roboflow) 은 사용자가 다른 터미널에서 `pull --rebase`+push 해 이미 origin(`1d74fce`). 재배치분은 이 세션에서 커밋·push.
