...
- **doosan-robot2 `-b jazzy` clone 의 DSR_ROBOT2.py 이름 불일치 2종 (2026-06-02 발견·패치)** — `from DSR_ROBOT2 import` 쓰는 모든 스크립트(jog/robot_control/pick_and_place) 공통으로 깨짐. `resources/dsr-project-install.sh` 2b 블록이 clone 직후 멱등 patch (colcon 빌드가 install 전파). 이미 설치된 머신은 `~/dsr_patch_command.txt` 직접 sed: (1) `SetSingularityHandlingForce`→`SetSingularHandlingForce` (존재 안 하는 service 클래스 참조 → 모듈 로드 NameError), (2) `_srv_name_prefix` `''`→`'dsr_controller2/'` (실서버가 `/dsr01/dsr_controller2/...` 인데 빈 prefix 라 `/dsr01/aux_control/...` 죽은 이름 호출 → `get_current_posj` 등 무한 대기). 검증: 실기에서 `get_current_posj()` 정상 반환.
- **실기 로봇 환경 (2026-06-02 검증)**: 모델 `m0609`, 네임스페이스 `dsr01`. 컨트롤러(DRCF) `192.168.1.100:12345`, 툴체인저(그리퍼) `192.168.1.1`, host `enp4s0 192.168.1.30/24`(로봇망). 실기 bringup: `ros2 launch dsr_bringup2 dsr_bringup2_rviz.launch.py mode:=real host:=192.168.1.100 port:=12345 model:=m0609 name:=dsr01 gui:=false`. bringup 후 실서버 전부 `/dsr01/dsr_controller2/...` 아래.
