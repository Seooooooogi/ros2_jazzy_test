...
# a02-about-project.sh — 컴퓨트/로봇/카메라 레이어 (DSR robot + RealSense camera + colcon 빌드).
#
...
if [[ "$(id -u)" -eq 0 ]]; then
    echo "a02: sudo 로 실행하지 마세요. 일반 사용자로 'bash a02-about-project.sh' 실행." >&2
...
run_step() {
    ...
    if step_should_skip "${name}"; then
        echo "[${n}/${A02_STEPS}] skip: ${name} (이미 DONE)"
        return 0
    fi
    step_begin "${n}" "${A02_STEPS}" "${name}"
    if "$@"; then
        step_end_ok
    else
        step_end_fail
