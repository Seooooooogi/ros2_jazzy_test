...
# resources/dsr-project-install.sh — Doosan DSR (doosan-robot2) driver clone + dependencies + emulator.
#
...
#   - workspace = ${DSR_WORKSPACE}(=~/cobot_ws). This script sets up only the DSR driver; doosan-robot2 is cloned
#     into ${DSR_WORKSPACE}/src. The cobot2 application source is NOT shipped by this repo — the user places it at
#     ${DSR_WORKSPACE}/src/cobot2 (setup-app.sh verifies its presence before the colcon build).
...
#   - rosdep update / colcon build are handled by colcon-build.sh (avoid duplicate builds).
# Pure install body — no state calls (setup-app.sh owns the step framing).
set -euo pipefail
...
# The cobot2 application source is NOT mirrored here — this repo no longer ships it. The user places it at
# ${WS_SRC}/cobot2 manually; setup-app.sh verifies its presence (fail-loud) before invoking colcon-build.sh.

# 3) apt packages for the DSR build dependency (only DSR-specific ones not in a01 ros2-install.sh / desktop core).
...
# 3b) Runtime Python deps of robot_control (host client) — installed via system Python (apt) as a thin client.
...
# 4) DSR emulator image (explicit tag — docker auto-skips if already present).
