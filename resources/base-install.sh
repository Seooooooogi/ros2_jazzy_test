...
base_kernel() {
    ...
    #    설치 전체가 멈춘다. wifi 모듈이 실제로 들어왔는지는 아래 3)이 보고 경고만 남긴다.
    local running extra
    running="$(uname -r)"
    extra="linux-modules-extra-${running}"
    if apt-cache show "${extra}" >/dev/null 2>&1; then
        sudo apt-get install -y "${extra}" "linux-headers-${running}"
    else
        echo "kernel: ${extra} not in apt — expecting the extra modules inside linux-modules-${running}."
