...
# 고정(pin) 방식: 소스 = 커밋 / 이미지 = 태그
#   미고정 시 설치 시점마다 다른 버전 설치 → 재현 불가
app_dsr() {
    ...
    if [[ -d "${WS_SRC}/doosan-robot2/.git" ]]; then
        ...
        # 절차 = 기본 브랜치로 clone → 핀 커밋으로 detach
        #   → fork 에 커밋이 얹혀도 설치 결과 불변
    ...
    # 3) 여기서 다루는 것 = DSR 전용 apt 패키지만
    #    나머지 의존성 = colcon 단계의 rosdep 이 해결
    sudo apt-get update
    ...
    #     venv 아님, apt 설치
    #       이유: 별도 활성화 없이 system Python 이 그대로 인식
    sudo apt-get install -y \
        python3-numpy python3-scipy python3-pymodbus

    # 4) DSR 에뮬레이터 이미지
    #    태그 명시 → latest 로 조용히 밀리는 것 차단
...
# RealSense 카메라 librealsense2 SDK(커널 모듈 + 유틸 + 헤더) 설치
#   apt 저장소·키링 등록 포함
...
#   컨테이너 미채택 이유: 마이크 = 하드웨어 종속
#     → 컨테이너로 오디오를 넘기는 방식이 머신마다 깨짐
app_voice() {
    local OWW_SRC="${SCRIPT_DIR}/oww_models"
    local WAKEWORD_MODEL="${VOICE_WS}/resource/hello_rokey_8332_32.tflite"

    # 아래에서 쓰는 ai-edge-litert = Python 3.12 wheel 만 존재
    #   → 다른 버전이면 이 지점에서 즉시 정지
    local PYVER
    ...
    # Ubuntu = system Python 을 apt 관리 대상으로 보고 pip 설치 차단
    #   → --break-system-packages 로 해제
    local PIP=(sudo python3 -m pip install --break-system-packages --no-cache-dir)

    # 2) 음성/LLM 스택
    #    scipy 1.18+ = numpy>=2 요구 → 마지막의 numpy<2 고정과 충돌 → 상한 설정
    ...
    #   현재 이 방식으로 걸리는 apt 파이썬 패키지 = typing-extensions 뿐
    #     증가 시 여기 한 줄 추가
    ...
    # 4) 위에서 빠진 실제 의존성 직접 설치
    #    tflite-runtime 자리 = API 동일한 ai-edge-litert 로 대체
    ...
    #    → 런타임이 받으려 할 모델을 설치 시점에 선행 배치
    #      기존 파일 skip → 런타임의 다운로드 호출이 무동작
    #    다운로드 일시 실패 → 에러 HTML 이 .tflite 이름으로 저장 → 런타임에서야 폭발
    #    → 수신 후 파일 시그니처 확인 + 손상본 삭제 + 이 지점에서 정지
    #      재실행 시 재다운로드
...
app_colcon() {
    ...
    # ROS2 환경 로드
    #   setup.bash = 미정의 변수 참조 → set -u 일시 해제 필요
    set +u
...
app_toolkit() {
    ...
    # 2) 키링 + apt source 등록
    #    설치 직전 update = 아래 3) 담당 → 여기선 --no-update
    ...
    #    nvidia-ctk = /etc/docker/daemon.json 갱신
    sudo nvidia-ctk runtime configure --runtime=docker

    # 5) 그 변경 반영 시점 = docker 재시작 후
    #    재시작 = 구동 중 컨테이너 정지 → 사전 동의 수령
