...
# Build scope is limited to the packages symlinked into ws src/ (dsr-project-install.sh only symlinks the host
# packages → container packages like object_detection / voice_processing are not in src/ and are auto-excluded).
...
# colcon build (packages in src/ = doosan-robot2 + host packages only).
colcon build
