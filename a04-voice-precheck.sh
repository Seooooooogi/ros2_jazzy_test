...
# 따라서 이 단계는 설치가 아니라, 컨테이너가 mount 할 .env 자격증명을 점검하는
# host 측 사전 점검이다 (app 이미지는 공개 Drive tar → docker load 라 레지스트리 로그인 불요).
...
# 단독 실행 시 스테이지-로컬 진행률 ([n/1]). 통합 실행(install.sh)은 자체 STEPS_TOTAL=15 사용.
STEPS_TOTAL=1
