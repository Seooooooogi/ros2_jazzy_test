...
- Entry (권장): `bash install.sh` — a01~a04 전체를 단일 시퀀스(`[n/17]`)로 실행. 완료된 step 은 자동 skip, reboot 후 재실행하면 이어서 진행. `--unattended`(reboot 자동·복귀 시 GUI autostart 로 자동 재개, 시작 시 OPENAI_API_KEY+confirm 1회·복귀 후 sudo 비번 1회), `--status`(상태), `--reset`(state 초기화), `--help`. 콘솔엔 `[n/total]` 진행률만 남고(경고/에러 포함 각 step 의 상세 출력 apt/pip/colcon 은 레포 루트 `install_log` append-only 로 빠진다 — `--verbose`/`VERBOSE=1` 로 콘솔 동시 출력). 단계 실패 시 콘솔엔 한 줄 `[FAIL]` + 로그 경로만.
