...
- 통합 실행 진입점: `bash containers/bringup.sh [launch args]` — host voice 노드 + yolo 컨테이너와 함께 `ros2 launch m0609_rg2_bringup bringup.launch.py` 실행(구 `cobot2_bringup bringup_all.launch.py` 대체). 사용자가 `camera:=` 를 안 주면 `camera:=true` 를 덧붙인다 — 새 launch 의 camera 기본값은 false 인데 yolo 노드는 `/camera/camera/*` 가 없으면 조용히 대기만 하기 때문.
